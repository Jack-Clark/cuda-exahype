...
#include "kernels/finitevolumes/godunov/c/2d/cuda_utils.cuh"
#include "kernels/finitevolumes/godunov/c/2d/userCode.cuh"


/* Responsible for updating d_Fn before the data is transferred back to the CPU for the final update of Fn. */
 ...
 * This version uses CUDA, and is the version that uses
 * streams. 
 */
extern "C"
double cudaSolutionUpdate(double* luh_new, double* luh, 
  ...
  gpuErrchk( cudaHostRegister(luh, cudaMemReq * sizeof(double), cudaHostRegisterDefault ));
  ...
  // Compute x edges
  eigenvalues_kernel<<<1, dimBlock, 0, stream1>>>(d_q, normalNonZeroIndex, d_sL, numberOfVariables, patchBegin, basisSize, 0);
  gpuErrchk( cudaPeekAtLastError() );
  eigenvalues_kernel<<<1, dimBlock, 0, stream2>>>(d_q, normalNonZeroIndex, d_sR, numberOfVariables, patchBegin, basisSize, 1);
  gpuErrchk( cudaPeekAtLastError() );
  ...
  compute_maxes_kernel<<<1, dimBlock, 0, stream1>>>(d_sL, d_sR, d_maxes, numberOfVariables);
  gpuErrchk( cudaPeekAtLastError() );

  double maxes[numCudaThreads];
  memset(maxes, 0, sizeof(double) * numCudaThreads);

  gpuErrchk( cudaMemcpyAsync(maxes, d_maxes, numCudaThreads * sizeof(double), cudaMemcpyDeviceToHost, stream1) );
  ...
  */
  ...
  flux_kernel<<<1, dimBlock, 0, stream2>>>(d_q, d_FL2, numberOfVariables, patchBegin, basisSize, 0);
  gpuErrchk( cudaPeekAtLastError() );
  flux_kernel<<<1, dimBlock, 0, stream3>>>(d_q, d_FR2, numberOfVariables, patchBegin, basisSize, 1);
  gpuErrchk( cudaPeekAtLastError() );
  ...
  cudaDeviceSynchronize();
  updateF_kernel<<<1, dimBlock, 0, stream1>>>(d_q, d_FL2, d_FR2, d_Fn, d_maxes, numberOfVariables, patchBegin, basisSize, normalNonZeroIndex, 1);
  gpuErrchk( cudaPeekAtLastError() );

  double Fn[numCudaThreads * numberOfVariables];
  gpuErrchk( cudaMemcpyAsync(Fn, d_Fn, numCudaThreads * numberOfVariables * sizeof(double), cudaMemcpyDeviceToHost, stream1) );
  

  // Compute y faces
  normalNonZeroIndex = 1;
  eigenvalues_kernel<<<1, dimBlock, 0, stream2>>>(d_q, normalNonZeroIndex, d_sL, numberOfVariables, patchBegin, basisSize, 2);
  gpuErrchk( cudaPeekAtLastError() );
  eigenvalues_kernel<<<1, dimBlock, 0, stream3>>>(d_q, normalNonZeroIndex, d_sR, numberOfVariables, patchBegin, basisSize, 3);
  gpuErrchk( cudaPeekAtLastError() );
  ...
  flux_kernel<<<1, dimBlock, 0, stream2>>>(d_q, d_FL2, numberOfVariables, patchBegin, basisSize, 2);
  gpuErrchk( cudaPeekAtLastError() );
  flux_kernel<<<1, dimBlock, 0, stream3>>>(d_q, d_FR2, numberOfVariables, patchBegin, basisSize, 3);
  gpuErrchk( cudaPeekAtLastError() );
  ...
  gpuErrchk( cudaHostUnregister(luh) );
