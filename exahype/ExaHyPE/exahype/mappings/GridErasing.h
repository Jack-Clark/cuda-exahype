...
#ifndef EXAHYPE_MAPPINGS_GridErasing_H_
#define EXAHYPE_MAPPINGS_GridErasing_H_
 ...
 */
class exahype::mappings::GridErasing {
  private:
    /**
  ...
  public:
     ...
     */
    GridErasing(const GridErasing& masterThread);
    #endif
     ...
     */   
    void mergeWithWorkerThread(const GridErasing& workerThread);
    #endif
 ...
 if ( fineGridVertex.isAdjacentToDomainOf( tarch::parallel::Node::getInstance().getRank() ) ) {
   ...
     ...
     *
     * More information on the interplay of ascend() and 
     * descend() with enterCell() and leaveCell() can be found in peano::grid::nodes::Refined.
     *
