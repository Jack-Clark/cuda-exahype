...
double cudaSolutionUpdate(double* luh_new, const double* luh, 
  ...
  // Compute x edges
  gen_eigenvalues_kernel<<<1, dimBlock, stream1>>>(d_q, normalNonZeroIndex, d_sL, numberOfVariables, patchBegin, basisSize, 0);
  gpuErrchk( cudaPeekAtLastError() );
  gen_eigenvalues_kernel<<<1, dimBlock, stream2>>>(d_q, normalNonZeroIndex, d_sR, numberOfVariables, patchBegin, basisSize, 1);
  gpuErrchk( cudaPeekAtLastError() );
  ...
  compute_maxes_kernel<<<1, dimBlock,stream1>>>(d_sL, d_sR, d_maxes, numberOfVariables);
  gpuErrchk( cudaPeekAtLastError() );

  double maxes[numCudaThreads];
  gpuErrchk( cudaHostRegister(maxes, numCudaThreads * sizeof(double), cudaHostRegisterDefault ));
  memset(maxes, 0, sizeof(double) * numCudaThreads);

  gpuErrchk( cudaMemcpAsync(maxes, d_maxes, numCudaThreads * sizeof(double), cudaMemcpyDeviceToHost, stream1) );
  ...
  */
  ...
  gen_flux_kernel_2D<<<1, dimBlock, stream2>>>(d_q, d_FL2, numberOfVariables, patchBegin, basisSize, 0);
  gpuErrchk( cudaPeekAtLastError() );
  gen_flux_kernel_2D<<<1, dimBlock, stream3>>>(d_q, d_FR2, numberOfVariables, patchBegin, basisSize, 1);
  gpuErrchk( cudaPeekAtLastError() );
  ...
  updateF_kernel<<<1, dimBlock, stream1>>>(d_q, d_FL2, d_FR2, d_Fn, d_maxes, numberOfVariables, patchBegin, basisSize, normalNonZeroIndex, 1);
  gpuErrchk( cudaPeekAtLastError() );

  double Fn[numCudaThreads * numberOfVariables];
  cudaHostRegister(Fn, sizeof(double) * numCudaThreads * numberOfVariables);
  gpuErrchk( cudaMemcpAsync(Fn, d_Fn, numCudaThreads * numberOfVariables * sizeof(double), cudaMemcpyDeviceToHost, stream1) );
  

  // Compute y faces
  normalNonZeroIndex = 1;
  gen_eigenvalues_kernel<<<1, dimBlock, stream2>>>(d_q, normalNonZeroIndex, d_sL, numberOfVariables, patchBegin, basisSize, 2);
  gpuErrchk( cudaPeekAtLastError() );
  gen_eigenvalues_kernel<<<1, dimBlock, stream3>>>(d_q, normalNonZeroIndex, d_sR, numberOfVariables, patchBegin, basisSize, 3);
  gpuErrchk( cudaPeekAtLastError() );
  ...
  gen_flux_kernel_2D<<<1, dimBlock, stream2>>>(d_q, d_FL2, numberOfVariables, patchBegin, basisSize, 2);
  gpuErrchk( cudaPeekAtLastError() );
  gen_flux_kernel_2D<<<1, dimBlock, stream3>>>(d_q, d_FR2, numberOfVariables, patchBegin, basisSize, 3);
  gpuErrchk( cudaPeekAtLastError() );
